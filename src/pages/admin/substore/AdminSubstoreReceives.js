...
export default function AdminSubstoreReceives(props) {
  ...
  const getReports = async () => {
    ...
    if (response.status === 200 && response.data) {
      ...
      setReceives(response.data.receives.data);
      setLinks(response.data.receives.links);
      setFrom(response.data.receives.from);
      setTo(response.data.receives.to);
      setTotal(response.data.receives.total);
    } else {
      console.log(response.data);
    }
    setSpinner(false);
      ...
      {userInfo.userId === 1 ? (
        <>
          <div className="create_page_heading">
            <div className="page_name">Receives</div>
                    ...
                    <tr key={index}>
                      <td>
                        ...
                        <button
                          ...
                          className="btn btn-sm btn-danger non_printing_area"
                        >
                          <i className="fa fa-times"></i>
                        </button>
                      </td>
                      <td>
                        <strong>{moment(item.created_at).format("lll")}</strong>
                      </td>
                      <td>
