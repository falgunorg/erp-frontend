...
export default function AdminSampleStore(props) {
  ...
  const getStores = async () => {
    setSpinner(true);
    var response = await api.post("/admin/sample/sample-stores", {
  ...
  const getStoreDetails = async (id) => {
    setSpinner(true);
    var response = await api.post("/admin/sample/sample-stores-show", {
      id: id,
    });
    if (response.status === 200 && response.data) {
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <Link
                            ...
                            onClick={() => getStoreDetails(item.id)}
                          >
                            Details
                          </Link>
                        </td>
