...
export default function SpecialSubstore(props) {
  ...
  // get all without paginatio store items
  ...
  const getAllItems = async () => {
    ...
    if (response.status === 200 && response.data) {
      setAllItems(response.data.substores_wo_pagination);
    } else {
  ...
  useEffect(async () => {
    getSubstores();
  }, [currentPage, searchValue, filterData, params]);

  useEffect(async () => {
    getAllItems();
  }, [filterData, searchValue, params]);
            ...
            <div className="col">
              <div className="form-group">
                ...
                <Select
                  placeholder="Select"
                  value={
                    allItems.find((item) => item.id === filterData.item_id)
                      ? {
              ...
              <tbody>
                {substores.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <Link to={`/sub-stores-details/${item.id}`}>
                        <strong> {item.last_purchase_qty}</strong> <br />
